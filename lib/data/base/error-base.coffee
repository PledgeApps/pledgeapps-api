...
class ErrorBase
	constructor: ( @id, @logDate, @errorMessage, @url ) ->
		@id = 0 if not @id?
	save: (cb) =>
		columns = { log_date: @logDate, error_message: @errorMessage, url: @url }
		if @id == 0
	...
	@delete = ( id, cb ) ->
		Global.query "DELETE FROM errors where id = " + id, null, (err, rows) =>
			cb();
	@loadRow = (row) ->
		return new ErrorBase row.id, row.log_date, row.error_message, row.url
