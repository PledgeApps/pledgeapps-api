...
Friend = require("../friend.coffee")

class FriendsBase extends Array
	constructor:  ->
	save: (cb) =>
	...
	sort_by: (field, reverse, primer) ->
		...
		sortFunction
	@loadAll = ( cb ) ->
		FriendsBase.loadFromQuery "select * from friends", {}, cb
	@loadFromQuery = ( query, params, cb ) ->
		Global.query query, params, (err, rows) =>
			sys.puts err if err?
			result = new FriendsBase()
			rows.forEach (row) ->
				result.push Friend.loadRow row
			cb(result);
